...
TypeId LteHexGridEnbTopologyHelper::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::LteHexGridEnbTopologyHelper").SetParent<Object>().AddConstructor<
					LteHexGridEnbTopologyHelper>().AddAttribute(
					"InterSiteDistance",
					"The distance [m] between nearby sites", DoubleValue(500),
					MakeDoubleAccessor(&LteHexGridEnbTopologyHelper::m_d),
					MakeDoubleChecker<double>()).AddAttribute("SectorOffset",
					...
					MakeDoubleChecker<double>()).AddAttribute("SiteHeight",
					"The height [m] of each site", DoubleValue(30),
					MakeDoubleAccessor(
							&LteHexGridEnbTopologyHelper::m_siteHeight),
					MakeDoubleChecker<double>()).AddAttribute("MinX",
					"The x coordinate where the hex grid starts.",
					DoubleValue(0.0),
					MakeDoubleAccessor(&LteHexGridEnbTopologyHelper::m_xMin),
					MakeDoubleChecker<double>()).AddAttribute("MinY",
					"The y coordinate where the hex grid starts.",
					DoubleValue(0.0),
					MakeDoubleAccessor(&LteHexGridEnbTopologyHelper::m_yMin),
					MakeDoubleChecker<double>()).AddAttribute("GridWidth",
					"The number of sites in even rows (odd rows will have one additional site).",
					UintegerValue(1),
					MakeUintegerAccessor(
							&LteHexGridEnbTopologyHelper::m_gridWidth),
...
NetDeviceContainer LteHexGridEnbTopologyHelper::SetPositionAndInstallEnbDevice(
		NodeContainer c) {
	NS_LOG_FUNCTION(this);
	...
	for (uint32_t n = 0; n < c.GetN(); ++n) {
		...
		NS_LOG_LOGIC(
				"node " << n << " site " << currentSite << " rowIndex " << rowIndex << " colIndex " << colIndex << " biRowIndex " << biRowIndex << " biRowRemainder " << biRowRemainder);
		...
		case 0:
			...
			x += m_offset;
			m_lteHelper->SetFfrAlgorithmAttribute("FrCellTypeId",
					UintegerValue(1));
			break;
		...
		NS_LOG_LOGIC(
				"node " << n << " at " << pos << " antennaOrientation " << antennaOrientation);
