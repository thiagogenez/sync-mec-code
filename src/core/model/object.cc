...
Object::AggregateIterator::AggregateIterator() :
		m_object(0), m_current(0) {
	NS_LOG_FUNCTION(this);
}

bool Object::AggregateIterator::HasNext(void) const {
	NS_LOG_FUNCTION(this);
	return m_current < m_object->m_aggregates->n;
}
Ptr<const Object> Object::AggregateIterator::Next(void) {
	NS_LOG_FUNCTION(this);
...
TypeId Object::GetInstanceTypeId(void) const {
	NS_LOG_FUNCTION(this);
	return m_tid;
}

TypeId Object::GetTypeId(void) {
	static TypeId tid =
...
Object::Object() :
		m_tid(Object::GetTypeId()), m_disposed(false), m_initialized(false), m_aggregates(
				(struct Aggregates *) std::malloc(sizeof(struct Aggregates))), m_getObjectCount(
				0) {
	NS_LOG_FUNCTION(this);
	m_aggregates->n = 1;
...
Object::Object(const Object &o) :
		m_tid(o.m_tid), m_disposed(false), m_initialized(false), m_aggregates(
				(struct Aggregates *) std::malloc(sizeof(struct Aggregates))), m_getObjectCount(
				0) {
	m_aggregates->n = 1;
	m_aggregates->buffer[0] = this;
...
void Object::Initialize(void) {
	 ...
	 */
	NS_LOG_FUNCTION(this);
	restart: uint32_t n = m_aggregates->n;
	for (uint32_t i = 0; i < n; i++) {
		Object *current = m_aggregates->buffer[i];
...
bool Object::IsInitialized(void) const {
	NS_LOG_FUNCTION(this);
...
void Object::UpdateSortedArray(struct Aggregates *aggregates,
		uint32_t j) const {
	NS_LOG_FUNCTION(this << aggregates << j);
	while (j > 0
			&& aggregates->buffer[j]->m_getObjectCount
...
void Object::AggregateObject(Ptr<Object> o) {
	...
	struct Aggregates *aggregates = (struct Aggregates *) std::malloc(
			sizeof(struct Aggregates) + (total - 1) * sizeof(Object*));
	aggregates->n = total;
	...
	for (uint32_t i = 0; i < other->m_aggregates->n; i++) {
		aggregates->buffer[m_aggregates->n + i] =
				other->m_aggregates->buffer[i];
		const TypeId typeId =
				other->m_aggregates->buffer[i]->GetInstanceTypeId();
		if (DoGetObject(typeId)) {
			NS_FATAL_ERROR(
					"Object::AggregateObject(): " "Multiple aggregation of objects of type " << other->GetInstanceTypeId () << " on objects of type " << typeId);
		}
 ...
 */
void Object::NotifyNewAggregate() {
	NS_LOG_FUNCTION(this);
}

Object::AggregateIterator Object::GetAggregateIterator(void) const {
	NS_LOG_FUNCTION(this);
...
void Object::DoInitialize(void) {
	NS_LOG_FUNCTION(this);
	NS_ASSERT(!m_initialized);
}

bool Object::Check(void) const {
	NS_LOG_FUNCTION(this);
